...
def test_suffix():
    """Verify that the suffix function returns a string and works correctly"""
    suf = suffix("working","fixing")
    assert isinstance(suf,str), "suffix function must return a string"
    
    #Calling suffix function nine times, and use an assert
    #statement to verify that the function returns the expected value
    assert suffix("","") == ""
